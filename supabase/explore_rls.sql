...
alter table if exists public.explore_comment_likes enable row level security;
...
create unique index if not exists explore_comment_likes_one_per_user_idx
  on public.explore_comment_likes (comment_id, user_id);
...
alter table if exists public.explore_comment_likes replica identity full;
...
begin
  ...
  begin
    alter publication supabase_realtime add table public.explore_comment_likes;
  exception
    when duplicate_object or undefined_object then null;
  end;
...
create or replace function public.explore_refresh_comment_like_count(target_comment_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
create or replace function public.explore_sync_comment_like_count()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  perform public.explore_refresh_comment_like_count(coalesce(new.comment_id, old.comment_id));
...
drop trigger if exists explore_comment_likes_sync_counts on public.explore_comment_likes;
create trigger explore_comment_likes_sync_counts
after insert or delete on public.explore_comment_likes
for each row execute function public.explore_sync_comment_like_count();
...
drop policy if exists "Comment likes are readable" on public.explore_comment_likes;
create policy "Comment likes are readable" on public.explore_comment_likes for select using (true);

drop policy if exists "Users manage own comment likes" on public.explore_comment_likes;
create policy "Users manage own comment likes" on public.explore_comment_likes for all
using (auth.uid() = user_id)
with check (auth.uid() = user_id);
