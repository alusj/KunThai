  ...
  return (
    <WorkspaceShell icon={UtensilsCrossed} eyebrow="Restaurant workspace" title={business.name} subtitle="Plan each day once; buyers only see the menu scheduled for their current day." stats={[{ label: "Today", value: DAYS[today].slice(0, 3) }, { label: "Items today", value: day === today ? items.length : "—" }, { label: "Selected day", value: DAYS[day].slice(0, 3) }, { label: "Available", value: items.filter((item) => item.available).length }]}>
      ...
      <section className="rounded-[26px] border border-gray-200 bg-white p-5 shadow-sm">
        <SectionHeading eyebrow={`${DAYS[day]} menu`} title="Meals buyers will see">{canManage ? <PrimaryButton onClick={openNewMeal} label="Add meal" className="bg-orange-600" /> : null}</SectionHeading>
        <div className="mt-5 grid gap-3 md:grid-cols-2">{loading ? <p className="text-sm font-bold text-gray-500">Loading menu...</p> : items.map((item) => <MealCard key={item.id} item={item} business={business} canManage={canManage} onEdit={() => editMeal(item)} onDelete={async () => { await deleteRestaurantMenuItem(item); await load(); notifyVerticalListingUpdated(business.id); showToast("Meal deleted.", "success"); }} onToggle={async () => { await toggleRestaurantMenuItem(item, !item.available); await load(); notifyVerticalListingUpdated(business.id); }} />)}</div>
...
function PropertyListingCard({ business, item, canManage = true, onDelete, onEdit }) {
  ...
  return <article className="relative rounded-2xl border border-gray-200 bg-white"><div className="absolute right-3 top-3 z-10"><SellerItemActions label={item.title} canManage={canManage} shareUrl={buildShareUrl("property", item.id)} onDelete={onDelete} onEdit={onEdit} /></div><MediaImage src={item.image_urls?.[0]} alt={item.title} className="h-44 w-full rounded-t-2xl object-cover" icon={House} /><div className="p-4"><span className="rounded-full bg-violet-50 px-2 py-1 text-[11px] font-black uppercase text-violet-700">For {item.purpose}</span><h3 className="mt-3 truncate pr-8 text-lg font-black">{item.title}</h3><p className="mt-1 flex items-center gap-1 text-sm font-bold text-gray-500"><MapPin size={15} /> {item.address}</p><div className="mt-3 flex gap-3 text-xs font-bold text-gray-500"><span className="flex gap-1"><BedDouble size={15} /> {item.bedrooms}</span><span className="flex gap-1"><Bath size={15} /> {item.bathrooms}</span><strong className="ml-auto text-gray-950">{business.currency} {Number(item.price).toLocaleString()}</strong></div><div className="mt-3 flex flex-wrap items-center gap-2 text-xs font-black text-gray-500"><span className="capitalize">{item.authorization_status}</span><span>·</span><span>{item.image_urls?.length || 0} images</span>{item.video_url ? <><span>·</span><span className="flex items-center gap-1"><Film size={13} /> Video</span></> : null}</div></div></article>;
}
