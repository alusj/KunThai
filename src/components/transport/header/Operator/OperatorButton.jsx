...
export default function OperatorButton({ badge = 0, hasCompanyAccount, hasOperatorAccount, onClick }) {
  ...
  if (hasOperatorAccount) {
    return <PremiumHeaderButton active accent="emerald" badge={badge} icon={Truck} iconSize={18} label={t("urride.header.openMyFleet")} onClick={onClick} />;
  }

  if (hasCompanyAccount) {
    return <PremiumHeaderButton active accent="emerald" badge={badge} icon={Building2} iconSize={18} label={t("urride.header.openFleetHq")} onClick={onClick} />;
  }

  return <PremiumHeaderButton active accent="emerald" icon={Plus} iconSize={18} label={t("urride.header.registerFleet")} onClick={onClick} />;
}
