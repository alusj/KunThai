...
export default function PostOptionsMenu({ isOwner, saved, onCopy, onDelete, onEdit, onHide, onReport, onSave, onShare, onViewActivity }) {
  ...
  return (
    <div className="absolute right-4 top-14 z-10 w-56 rounded-[18px] border border-slate-200 bg-white p-2 text-left shadow-xl">
      {(isOwner ? ownerActions : viewerActions).map((item) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={item.action}
            className={`flex w-full items-center gap-2 rounded-[14px] px-3 py-2 text-sm font-bold transition hover:bg-slate-100 ${
