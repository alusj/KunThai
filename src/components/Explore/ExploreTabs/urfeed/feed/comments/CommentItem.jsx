        ...
        <div className="min-w-0 flex-1 rounded-[20px] bg-slate-50 px-4 py-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-slate-400">@{comment.author_username || "user"} · {formatRelativeTime(comment.created_at)}</p>
            </div>
          </div>

          {comment.body ? <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{comment.body}</p> : null}
          {comment.audio_url ? <audio controls src={comment.audio_url} className="mt-3 h-10 w-full" /> : null}
