...
export default function AdminMfaGate({ children, bypass = false }) {
  ...
  async function beginEnrollment() {
    ...
    try {
      ...
      if (factors.error) {
        throw new Error(factors.error.message || "Unable to list MFA factors.");
      }
      ...
      for (const factor of totpFactors) {
        if (factor.status !== "verified") {
          const remove = await supabase.auth.mfa.unenroll({
            factorId: factor.id,
          });

          if (remove.error) {
            console.warn("Unable to remove stale MFA factor:", remove.error);
          }
        }
      }
      ...
      const result = await supabase.auth.mfa.enroll({
        factorType: "totp",
        friendlyName: `KunThai Admin ${Date.now()}`,
      });
      ...
      <section className="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        {state.mode === "enroll" ? (
          <div className="mt-4">
            ...
            <button
              ...
              disabled={busy}
              className="mt-6 inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-zinc-950 px-4 text-sm font-black text-white hover:bg-zinc-800 disabled:opacity-60"
            >
              {busy ? (
                <LoaderCircle className="animate-spin" size={18} />
              ) : (
        ...
        {state.mode === "verify-enrollment" ? (
          <div className="mt-5">
            ...
            <p className="mt-3 break-all rounded-md bg-zinc-100 px-3 py-2 text-center font-mono text-xs text-zinc-700">
              {state.secret}
            </p>
          </div>
        ) : null}
          ...
          <div className="mt-6 space-y-4">
            <label className="block">
              ...
              <input
                ...
                className="h-12 w-full rounded-lg border border-zinc-300 px-3 text-center font-mono text-xl font-black text-zinc-950 focus:border-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-100"
              />
            </label>
            ...
            <button
              ...
              className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-zinc-950 px-4 text-sm font-black text-white hover:bg-zinc-800 disabled:opacity-60"
            >
              {busy ? (
                <LoaderCircle className="animate-spin" size={18} />
              ) : (
